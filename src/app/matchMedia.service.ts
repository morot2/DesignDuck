...
export class MatchMediaService
{
  ...
  rules =
    {
      ...
      tablet: '(min-width: 768px) and (max-width: 800px)',
  ...
  OnPhone(callBack) {
    if (typeof callBack === 'function') {
      let mql: MediaQueryList = window.matchMedia(this.rules.phone);
  ...
  OnTablet(callBack)
  {
    if (typeof callBack === 'function')
    {
      let mql: MediaQueryList = window.matchMedia(this.rules.tablet);
  ...
  OnDesktop(callBack)
  {
    if (typeof callBack === 'function')
    {
      let mql: MediaQueryList = window.matchMedia(this.rules.desktop);
  ...
  OnPortrait(callBack)
  {
    if (typeof callBack === 'function')
    {
      let mql: MediaQueryList = window.matchMedia(this.rules.portrait);
  ...
  OnLandscape(callBack)
  {
    if (typeof callBack === 'function')
    {
      let mql: MediaQueryList = window.matchMedia(this.rules.landscape);
